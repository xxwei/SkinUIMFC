...
VOID CSkinHeaderCtrl::OnPaint()
{
	...
	BufferDC.SetTextColor(m_colNormalText_ListCtrl); //gucz0901
...
CSkinListCtrl::CSkinListCtrl()
{
    ...
    m_colBack = RGB(0xBA, 0xBA, 0xBA);//gucz0904:修改liblist的窗口背景为#BABABA?
	m_bDragging = false;
...
void CSkinListCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (m_bDragging)
	{
		...
		m_pDragImageList->DragMove(point); // move the image

		if ((iItem = HitTest(point)) != -1)
		{
			m_iItemDrop = iItem;
			m_pDragImageList->SetDragCursorImage(1, CPoint(4, 4));
		}
		else {
			m_pDragImageList->SetDragCursorImage(-1, CPoint(0, 0));
		}
	}
	else
	{
...
void CSkinListCtrl::OnLvnBegindrag(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	ASSERT(m_pDragImageList == NULL);
	m_pDragImageList = CreateDragImage(m_iItemDrag, &ptImage);
	IMAGEINFO imageInfo;
	m_pDragImageList->GetImageInfo(0, &imageInfo);
	m_ptHotSpot = ptAction - ptItem + m_ptOrigin; // calculate hotspot for the cursor
	m_pDragImageList->DragShowNolock(TRUE); // lock updates and show drag image
	//m_pDragImageList->Add(((HICON)LoadImage(NULL, _T("test.ico"), IMAGE_ICON, 92, 68, LR_LOADFROMFILE | LR_LOADTRANSPARENT)));
	...
	m_pDragImageList->DragMove(ptAction); // move image to overlap original icon
	SetCapture();
	CRect rect;
	GetClientRect(&rect);
	ClientToScreen(&rect);
...
void CSkinListCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (m_bDragging) // end of the drag operation
	{
		...
		if (m_iItemDrop == m_iItemDrag)
		{
			...
			//bool ret = GetItemPosition(m_iItemDrop, &pointDrop);
			//ret = GetItemPosition(m_iItemDrag, &pointDrag);
			//ret = SetItemPosition(m_iItemDrag, pointDrop);
			//ret = SetItemPosition(m_iItemDrop, pointDrag);
			SwapItem(m_iItemDrop, m_iItemDrag);
		}
		m_bDragging = FALSE;
		Invalidate();
		::ReleaseCapture();
...
void CSkinListCtrl::SwapItem(INT srcitem, INT dstitem)
{
	int Count = m_SkinHeaderCtrl.GetItemCount();
	for (int i = 0; i < Count; i++)
	{
		CString srcstr = GetItemText(srcitem, i);
		CString dststr = GetItemText(dstitem, i);
		SetItemText(srcitem, i, dststr);
		SetItemText(dstitem, i, srcstr);
	}
	DWORD_PTR srcdata = GetItemData(srcitem);
	DWORD_PTR dstdata = GetItemData(dstitem);
	SetItemData(dstitem, srcdata);
	SetItemData(srcitem, dstdata);
	Invalidate();
}
