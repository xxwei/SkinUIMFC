...
CSkinButton::~CSkinButton()
{
	SafeDelete(m_pBackImgN);
	SafeDelete(m_pBackImgH);
	SafeDelete(m_pBackImgD);
	SafeDelete(m_pBackImgF);
	SafeDelete(m_pCheckImgN);
	SafeDelete(m_pCheckImgH);
	SafeDelete(m_pCheckImgTickN);
	SafeDelete(m_pCheckImgTickH);
...
void CSkinButton::DrawPushButton(CDC* pDC,RECT &rcClient)
{
	...
	else if (IsWindowEnabled()==false) //不可用状态
	{
		if (m_pUnEnableImg != NULL && !m_pUnEnableImg->IsNull())
			m_pUnEnableImg->DrawImage(pDC, rcClient);
	}
	else				// 普通状态
	{
	...
	else if (bHasText)	// 仅文字
	{
		...
		{
			pDC->SelectObject(RenderEngine->GetFont(nFontIndex - 1));
		}
...
void CSkinButton::SetUpDown(bool bDown)
{
	m_bPress = bDown;
...
void CSkinButton::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
{
	...
	else if (_tcscmp(pstrName, _T("unenableimage")) == 0)
	{
		RenderEngine->RemoveImage(m_pUnEnableImg);
		m_pUnEnableImg = RenderEngine->GetImage(pstrValue);
	...
	{
		RenderEngine->RemoveImage(m_pBackImgN);
		m_pBackImgN = RenderEngine->GetImage(pstrValue);
	}
	else if( _tcscmp(pstrName, _T("hotimage")) == 0 ) 
	{
		RenderEngine->RemoveImage(m_pBackImgH);
		m_pBackImgH = RenderEngine->GetImage(pstrValue);
	}
	else if( _tcscmp(pstrName, _T("pushedimage")) == 0 ) 
	{
		RenderEngine->RemoveImage(m_pBackImgD);
		m_pBackImgD = RenderEngine->GetImage(pstrValue);
	}
	else if( _tcscmp(pstrName, _T("focusedimage")) == 0 ) 
	{
		RenderEngine->RemoveImage(m_pBackImgF);
		m_pBackImgF = RenderEngine->GetImage(pstrValue);
	}
	//check
	else if( _tcscmp(pstrName, _T("checkimgnormal")) == 0 ) 
	{
		RenderEngine->RemoveImage(m_pCheckImgN);
		m_pCheckImgN = RenderEngine->GetImage(pstrValue);
	}
	else if( _tcscmp(pstrName, _T("checkimghot")) == 0 ) 
	{
		RenderEngine->RemoveImage(m_pCheckImgH);
		m_pCheckImgH = RenderEngine->GetImage(pstrValue);
	}
	else if( _tcscmp(pstrName, _T("checkimgtickn")) == 0 ) 
	{
		RenderEngine->RemoveImage(m_pCheckImgTickN);
		m_pCheckImgTickN = RenderEngine->GetImage(pstrValue);
	}
	else if( _tcscmp(pstrName, _T("checkimgtickh")) == 0 ) 
	{
		RenderEngine->RemoveImage(m_pCheckImgTickH);
		m_pCheckImgTickH = RenderEngine->GetImage(pstrValue);
	}
	else if( _tcscmp(pstrName, _T("iconimage")) == 0 ) 
	{
		RenderEngine->RemoveImage(m_pIconImg);
		m_pIconImg = RenderEngine->GetImage(pstrValue);
	}
	else if( _tcscmp(pstrName, _T("menuimage")) == 0 ) 
	{
		RenderEngine->RemoveImage(m_pArrowImg);
		m_pArrowImg = RenderEngine->GetImage(pstrValue);
	}
