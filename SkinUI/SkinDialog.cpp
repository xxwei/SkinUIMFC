...
{
	if (m_bDefaultSkin) return __super::OnGetMinMaxInfo(lpMMI);
...
void CSkinDialog::RectifyControl(int cx, int cy)
{
	...
	for (int i = 0; i < (int)m_ControlArray.size(); i++)
	{
		...
		if (pControl != NULL)
		{
			...
			if (_tcscmp(TEXT("msctls_trackbar32"), szClassName) == 0)
			{
				CSkinSliderCtrl *pSliderCtrl = (CSkinSliderCtrl*)pControl->m_pOwnWnd;
				pSliderCtrl->SetChannelRect();
			}
		}
	}
